# Installation Guide for param Code with Source Tracking

This guide describes how to install Node.js and param Code with source information tracking.

## Overview

The installation scripts automate the process of installing Node.js (if not present or below version 20) and param Code, while capturing and storing the installation source information for analytics and tracking purposes.

## Installation Scripts

We provide platform-specific installation scripts:

- **Linux/macOS**: `install-param-with-source.sh`
- **Windows**: `install-param-with-source.bat`

## Linux/macOS Installation

### Script: install-param-with-source.sh

#### Features:

- Checks for existing Node.js installation and version
- Installs Node.js 20+ if needed using NVM
- Installs param Code globally with source information
- Stores the source information in `~/.param/source.json`

#### Usage:

```bash
# Install with a specific source
sh install-param-with-source.sh --source github

# Install with internal source
sh install-param-with-source.sh -s internal

# Show help
sh install-param-with-source.sh --help
```
...
1. The script accepts a `--source` parameter to specify where param Code is being installed from
2. It installs Node.js if needed
3. It installs param Code globally
4. It creates `~/.param/source.json` with the specified source information
...
This is required to load the newly installed Node.js and param Code into your PATH.
...
### Script: install-param-with-source.bat
...
- Installs param Code globally with source information
- Stores the source information in `%USERPROFILE%\.param\source.json`

#### Prerequisites:

- **PowerShell (Administrator)**: The script must be run in PowerShell with Administrator privileges
- Internet connection for downloading Node.js and param Code
...
./install-param-with-source.bat --source github

# Install with short parameter
./install-param-with-source.bat -s internal

# Use default source (unknown)
./install-param-with-source.bat
```
...
1. The script accepts a `--source` or `-s` parameter to specify where param Code is being installed from
...
4. It installs param Code globally using npm
5. It creates `%USERPROFILE%\.param\source.json` with the specified source information
...
This feature implements the ability to capture and store the installation source of the param Code package. The source information is used for analytics and tracking purposes.

### Storage Location

The installation source is stored in a separate file at:

- **Unix/Linux/macOS**: `~/.param/source.json`
- **Windows**: `%USERPROFILE%\.param\source.json` (equivalent to `C:\Users\{username}\.param\source.json`)
...
2. **Analytics**: Helps understand how users are discovering and installing param Code
...
- The `paramLogger` class reads this file during telemetry initialization
...
```bash
cat ~/.param/source.json
```

**Windows:**

```cmd
type %USERPROFILE%\.param\source.json
```
...
./install-param-with-source.bat -s github
```
...
- Ensure NVM is installed: `curl -o- https://param-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh | bash`
...
- The scripts require internet access to download Node.js and param Code
