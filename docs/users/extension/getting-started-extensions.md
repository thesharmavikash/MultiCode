# Getting Started with param Code Extensions

This guide will walk you through creating your first param Code extension. You'll learn how to set up a new extension, add a custom tool via an MCP server, create a custom command, and provide context to the model with a `param.md` file.

## Prerequisites

Before you start, make sure you have the param Code installed and a basic understanding of Node.js and TypeScript.
...
```bash
param extensions new my-first-extension mcp-server
```
...
This is the manifest file for your extension. It tells param Code how to load and use your extension.
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP) servers. MCP servers are how you can add new tools for the model to use.
  - `command`, `args`, `cwd`: These fields specify how to start your server. Notice the use of the `${extensionPath}` variable, which param Code replaces with the absolute path to your extension's installation directory. This allows your extension to work regardless of where it's installed.
...
Before you can use the extension, you need to compile the TypeScript code and link the extension to your param Code installation for local development.
...
2.  **Build the server:**
    ...
    This will compile `example.ts` into `dist/example.js`, which is the file referenced in your `param-extension.json`.

3.  **Link the extension:**

    The `link` command creates a symbolic link from the param Code extensions directory to your development directory. This means any changes you make will be reflected immediately without needing to reinstall.

    ```bash
    param extensions link .
    ```

Now, restart your param Code session. The new `fetch_posts` tool will be available. You can test it by asking: "fetch posts".
...
After saving the file, restart the param Code. You can now run `/fs:grep-code "some pattern"` to use your new command.

## Step 5: Add Custom Skills and Subagents (Optional)

Extensions can also provide custom skills and subagents to extend param Code's capabilities.
...
After restarting param Code, your custom skills will be available via `/skills` and subagents via `/agents manage`.

## Step 6: Add a Custom `param.md`

You can provide persistent context to the model by adding a `param.md` file to your extension. This is useful for giving the model instructions on how to behave or information about your extension's tools. Note that you may not always need this for extensions built to expose commands and prompts.

1.  Create a file named `param.md` in the root of your extension directory:
...
2.  Update your `param-extension.json` to tell the CLI to load this file:
...
Restart the CLI again. The model will now have the context from your `param.md` file in every session where the extension is active.
...
You've successfully created a param Code extension! You learned how to:
...
From here, you can explore more advanced features and build powerful new capabilities into the param Code.
