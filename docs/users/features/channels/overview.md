...
- Approved users are stored in `~/.param/channels/<name>-allowlist.json` — treat this file as sensitive
...
When you run `param channel start` without a name, all channels defined in `settings.json` start together sharing a single agent process. Each channel maintains its own sessions — a Telegram user and a WeChat user get separate conversations, even though they share the same agent.
...
- On clean shutdown (Ctrl+C or `param channel stop`): session data is cleared — the next start is always fresh
