# param Code Hooks Documentation

## Overview

param Code hooks provide a powerful mechanism for extending and customizing the behavior of the param Code application. Hooks allow users to execute custom scripts or programs at specific points in the application lifecycle, such as before tool execution, after tool execution, at session start/end, and during other key events.
...
Hooks are user-defined scripts or programs that are automatically executed by param Code at predefined points in the application flow. They allow users to:
...
The param Code hook system consists of several key components:
...
The following table lists all available hook events in param Code:
...
| `Stop`               | Fired before param concludes its response    | Finalization, cleanup                           |
...
**Purpose**: Executed before param concludes its response to provide final feedback or summaries.
...
{
  "decision": "block",
  "reason": "Must be provided when param Code is blocked from stopping"
}
```
...
Hooks are configured in param Code settings, typically in `.param/settings.json` or user configuration files:
...
echo "INFO: Tool $TOOL_NAME executed safely at $(date)" >> /var/log/param-security.log
...
Configure in `.param/settings.json`:
