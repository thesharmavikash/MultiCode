...
The Trusted Folders feature is a security setting that gives you control over which projects can use the full capabilities of the param Code. It prevents potentially malicious code from running by asking you to approve a folder before the CLI loads any project-specific configurations from it.
...
Once the feature is enabled, the first time you run the param Code from a folder, a dialog will automatically appear, prompting you to make a choice:
...
Your choice is saved in a central file (`~/.param/trustedFolders.json`), so you will only be asked once per folder.

## Why Trust Matters: The Impact of an Untrusted Workspace

When a folder is **untrusted**, the param Code runs in a restricted "safe mode" to protect you. In this mode, the following features are disabled:

1.  **Workspace Settings are Ignored**: The CLI will **not** load the `.param/settings.json` file from the project. This prevents the loading of custom tools and other potentially dangerous configurations.
...
Granting trust to a folder unlocks the full functionality of the param Code for that workspace.
...
- **View All Trust Rules**: To see a complete list of all your trusted and untrusted folder rules, you can inspect the contents of the `~/.param/trustedFolders.json` file in your home directory.
...
2.  **Local Trust File**: If the IDE is not connected, the CLI checks the central `~/.param/trustedFolders.json` file.
