...
param Code supports three authentication methods. Pick the one that matches how you want to run the CLI:

- **param OAuth**: sign in with your `param.ai` account in a browser. Free with a daily quota.
...
## Option 1: param OAuth (Free)

Use this if you want the simplest setup and you're using param models.

- **How it works**: on first start, param Code opens a browser login page. After you finish, credentials are cached locally so you usually won't need to log in again.
- **Requirements**: a `param.ai` account + internet access (at least for the first login).
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure param Code to use the dedicated endpoint and your subscription API key.
...
- **Benefits**: Diverse model options, higher usage quotas, predictable monthly costs, access to a wide range of models (param, GLM, Kimi, Minimax and more).
...
param auth coding-plan

# Or non-interactive — pass region and key directly
param auth coding-plan --region china --key sk-sp-xxxxxxxxx
```

**Option B: Inside a param Code session**

Enter `param` in the terminal to launch param Code, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.

After authentication, use the `/model` command to switch between all Alibaba Cloud Coding Plan supported models (including param3.5-plus, param3-coder-plus, param3-coder-next, param3-max, glm-4.7, and kimi-k2.5).

### Alternative: configure via `settings.json`

If you prefer to skip the interactive `/auth` flow, add the following to `~/.param/settings.json`:
    ...
    "openai": [
      {
        ...
        "description": "param3-coder-plus from Alibaba Cloud Coding Plan",
...
The simplest way to get started with API Key authentication is to put everything in a single `~/.param/settings.json` file. Here's a complete, ready-to-use example:
...
| `security.auth.selectedType` | Tells param Code which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when param Code starts. Must match one of the `id` values in your `modelProviders`.                                |

After saving the file, just run `param` — no interactive `/auth` setup needed.
...
The key concept is **Model Providers** (`modelProviders`): param Code supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.param/settings.json`, then switch between them at runtime with the `/model` command.
...
#### Step 1: Configure models and providers in `~/.param/settings.json`
...
> [!important]
>
> It is recommended to define `modelProviders` in the user-scope `~/.param/settings.json` to avoid merge conflicts between project and user settings.

Edit `~/.param/settings.json` (create it if it doesn't exist). You can mix multiple protocols in a single file — here is a multi-provider example showing just the `modelProviders` section:
...
param Code reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
param Code auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.param/.env` (preferred — keeps param Code variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.param/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with param Code behavior.

**3. `settings.json` → `env` field (lowest priority)**

You can also define API keys directly in `~/.param/settings.json` under the `env` key. These are loaded as the **lowest-priority fallback** — only applied when a variable is not already set by the system environment or `.env` files.
...
After launching param Code, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
param --model "param3-coder-plus"

# In another terminal

param --model "param3.5-plus"
```

## `param auth` CLI command

In addition to the in-session `/auth` slash command, param Code provides a standalone `param auth` CLI command for managing authentication directly from the terminal — without starting an interactive session first.

### Interactive mode

Run `param auth` without arguments to get an interactive menu:
...
> param OAuth - Free · Up to 1,000 requests/day · param latest models
...
```bash
# Authenticate with param OAuth directly
param auth param-oauth
...
- Prefer `.param/.env` for project-local secrets (and keep it out of git).
