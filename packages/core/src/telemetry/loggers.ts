...
export function logStartSession(
  ...
  ParamLogger.getInstance(config)?.logStartSessionEvent(event);
...
export function logUserPrompt(config: Config, event: UserPromptEvent): void {
  ParamLogger.getInstance(config)?.logNewPromptEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserRetry(config: Config, event: UserRetryEvent): void {
  ParamLogger.getInstance(config)?.logRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolCall(config: Config, event: ToolCallEvent): void {
  ...
  ParamLogger.getInstance(config)?.logToolCallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolOutputTruncated(
  ...
  ParamLogger.getInstance(config)?.logToolOutputTruncatedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFileOperation(
  ...
  ParamLogger.getInstance(config)?.logFileOperationEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiRequest(config: Config, event: ApiRequestEvent): void {
  // ParamLogger.getInstance(config)?.logApiRequestEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFlashFallback(
  ...
  ParamLogger.getInstance(config)?.logFlashFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logRipgrepFallback(
  ...
  ParamLogger.getInstance(config)?.logRipgrepFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiError(config: Config, event: ApiErrorEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
  ParamLogger.getInstance(config)?.logApiErrorEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiCancel(config: Config, event: ApiCancelEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  ParamLogger.getInstance(config)?.logApiCancelEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiResponse(config: Config, event: ApiResponseEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
  ParamLogger.getInstance(config)?.logApiResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logLoopDetected(
  ...
  ParamLogger.getInstance(config)?.logLoopDetectedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logLoopDetectionDisabled(
  ...
  ParamLogger.getInstance(config)?.logLoopDetectionDisabledEvent();
}
...
export function logNextSpeakerCheck(
  ...
  ParamLogger.getInstance(config)?.logNextSpeakerCheck(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSlashCommand(
  ...
  ParamLogger.getInstance(config)?.logSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logIdeConnection(
  ...
  ParamLogger.getInstance(config)?.logIdeConnectionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logConversationFinishedEvent(
  ...
  ParamLogger.getInstance(config)?.logConversationFinishedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logChatCompression(
  ...
  ParamLogger.getInstance(config)?.logChatCompressionEvent(event);
...
export function logKittySequenceOverflow(
  ...
  ParamLogger.getInstance(config)?.logKittySequenceOverflowEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logMalformedJsonResponse(
  ...
  ParamLogger.getInstance(config)?.logMalformedJsonResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logInvalidChunk(
  ...
  ParamLogger.getInstance(config)?.logInvalidChunkEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetry(
  ...
  ParamLogger.getInstance(config)?.logContentRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetryFailure(
  ...
  ParamLogger.getInstance(config)?.logContentRetryFailureEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSubagentExecution(
  ...
  ParamLogger.getInstance(config)?.logSubagentExecutionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logModelSlashCommand(
  ...
  ParamLogger.getInstance(config)?.logModelSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logHookCall(config: Config, event: HookCallEvent): void {
  // Log to paramLogger for RUM telemetry only
  ParamLogger.getInstance(config)?.logHookCallEvent(event);
}
...
export function logExtensionInstallEvent(
  ...
  ParamLogger.getInstance(config)?.logExtensionInstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionUninstall(
  ...
  ParamLogger.getInstance(config)?.logExtensionUninstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export async function logExtensionUpdateEvent(
  ...
  ParamLogger.getInstance(config)?.logExtensionUpdateEvent(event);
...
export function logExtensionEnable(
  ...
  ParamLogger.getInstance(config)?.logExtensionEnableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionDisable(
  ...
  ParamLogger.getInstance(config)?.logExtensionDisableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logAuth(config: Config, event: AuthEvent): void {
  ParamLogger.getInstance(config)?.logAuthEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserFeedback(
  ...
  uiTelemetryService.addEvent(uiEvent);
  config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
  ParamLogger.getInstance(config)?.logUserFeedbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionStarted(
  ...
  ParamLogger.getInstance(config)?.logArenaSessionStartedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaAgentCompleted(
  ...
  ParamLogger.getInstance(config)?.logArenaAgentCompletedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionEnded(
  ...
  ParamLogger.getInstance(config)?.logArenaSessionEndedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
