...
describe('ReadFileTool', () => {
  ...
  describe('execute', () => {
    ...
    describe('with .paramignore', () => {
      ...
      it('should throw error if path is ignored by a .paramignore pattern', async () => {
        const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
        await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
        const params: ReadFileToolParams = {
          file_path: ignoredFilePath,
        };
        const expectedError = `File path '${ignoredFilePath}' is ignored by .paramignore pattern(s).`;
