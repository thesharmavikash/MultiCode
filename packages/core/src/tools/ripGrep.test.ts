...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass .paramignore to ripgrep when respected', async () => {
      await fs.writeFile(
    ...
    it('should include .paramignore matches when disabled in config', async () => {
      await fs.writeFile(path.join(tempRootDir, '.paramignore'), 'kept.txt\n');
  ...
  describe('multi-directory workspace', () => {
    ...
    it('should load .paramignore from each workspace directory', async () => {
      const secondDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'grep-tool-second-'),
      );
      await fs.writeFile(path.join(secondDir, '.paramignore'), 'ignored.txt\n');
      await fs.writeFile(
        path.join(tempRootDir, '.paramignore'),
        'other-ignored.txt\n',
      );
      ...
      // Verify both .paramignore files were passed
      ...
      expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.paramignore'));
      expect(ignoreFileArgs).toContain(path.join(secondDir, '.paramignore'));
