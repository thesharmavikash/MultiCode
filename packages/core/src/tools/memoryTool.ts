  ...
  parametersJsonSchema: {
    ...
    properties: {
      ...
      scope: {
        type: 'string',
        description:
          'Where to save the memory: "global" saves to user-level ~/.param/param.md (shared across all projects), "project" saves to current project\'s param.md (project-specific). If not specified, will prompt user to choose.',
        enum: ['global', 'project'],
      },
    },
    required: ['fact'],
...
- \`scope\` (string, optional): Where to save the memory:
  - "global": Saves to user-level ~/.param/param.md (shared across all projects)
  - "project": Saves to current project's param.md (project-specific)
  - If not specified, the tool will ask the user where they want to save the memory.
`;

export const DEFAULT_CONTEXT_FILENAME = 'param.md';
export const AGENT_CONTEXT_FILENAME = 'AGENTS.md';
export const MEMORY_SECTION_HEADER = '## param Added Memories';

// This variable will hold the currently configured filename for context files.
// It defaults to include both param.md and AGENTS.md but can be overridden by setGeminiMdFilename.
// param.md is first to maintain backward compatibility (used by /init command and save_memory tool).
...
function getGlobalMemoryFilePath(): string {
  return path.join(Storage.getGlobalParamDir(), getCurrentGeminiMdFilename());
}
