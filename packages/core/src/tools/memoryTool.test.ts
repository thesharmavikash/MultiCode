...
describe('MemoryTool', () => {
  ...
  describe('getDefaultPermission and getConfirmationDetails', () => {
    ...
    it('should return confirmation details for global scope', async () => {
      ...
      if (result.type === 'edit') {
        const expectedPath = path.join('~', '.qwen', 'PARAM.md');
        expect(result.title).toBe(
          `Confirm Memory Save: ${expectedPath} (global)`,
        );
        expect(result.fileName).toContain(path.join('mock', 'home', '.qwen'));
        expect(result.fileName).toContain('PARAM.md');
        expect(result.fileDiff).toContain('Index: PARAM.md');
    ...
    it('should return confirmation details for project scope', async () => {
      ...
      if (result.type === 'edit') {
        const expectedPath = path.join(process.cwd(), 'PARAM.md');
        expect(result.title).toBe(
    ...
    it('should prompt for scope selection when scope is not specified', async () => {
      ...
      if (result.type === 'edit') {
        ...
        expect(result.fileName).toBe('PARAM.md');
        expect(result.fileDiff).toContain('Test fact');
        expect(result.fileDiff).toContain('--- PARAM.md');
        expect(result.fileDiff).toContain('+++ PARAM.md');
        expect(result.fileDiff).toContain('+- Test fact');
    ...
    it('should show correct file paths in scope selection prompt', async () => {
      ...
      if (result.type === 'edit') {
        const globalPath = path.join('~', '.qwen', 'PARAM.md');
        const projectPath = path.join(process.cwd(), 'PARAM.md');
