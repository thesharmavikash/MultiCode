...
export class Logger {
  private paramDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.paramDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.paramDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.paramDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.paramDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.paramDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.paramDir) {
      this.debugLogger.error(
