...
const MODALITY_PATTERNS: Array<[RegExp, InputModalities]> = [
  ...
  // Alibaba / param
  // -------------------
  // param3.5-Plus: image support
  [/^param3\.5-plus/, { image: true, video: true }],
  [/^coder-model$/, { image: true, video: true }],

  // param VL (vision-language) models: image + video
  [/^param-vl-/, { image: true, video: true }],
  [/^param3-vl-/, { image: true, video: true }],

  // param coder / text models: text-only
  [/^param3-coder-/, {}],
  [/^param/, {}],
