...
/**
 * Creates a mock ParamOAuth2Client for testing
 */
function createMockParamClient(
  initialCredentials: Partial<paramCredentials> = {},
): IParamOAuth2Client {
  let credentials: paramCredentials = {
  ...
  return {
    setCredentials: vi.fn((creds: paramCredentials) => {
...
function createValidCredentials(
  overrides: Partial<paramCredentials> = {},
): paramCredentials {
  return {
...
describe('SharedTokenManager', () => {
  ...
  beforeEach(() => {
    ...
    mockPath.dirname.mockImplementation((filePath) => {
      ...
      if (!filePath || typeof filePath !== 'string') {
        return '/home/user/.Param'; // Return the expected directory path
      }
  ...
  describe('getValidCredentials', () => {
    it('should return valid cached credentials without refresh', async () => {
      const mockClient = createMockParamClient();
    ...
    it('should refresh expired credentials', async () => {
      const mockClient = createMockParamClient(createExpiredCredentials());
    ...
    it('should force refresh when forceRefresh is true', async () => {
      const mockClient = createMockParamClient(createValidCredentials());
      const refreshResponse = createSuccessfulRefreshResponse();
  ...
  describe('CredentialsClearRequiredError handling', () => {
    it('should clear memory cache when CredentialsClearRequiredError is thrown during refresh', async () => {
      ...
      const mockClient = {
        ...
        getProviderConfig: vi.fn(() => PARAM_OAUTH_CONFIG),
        getAccessToken: vi.fn(),        requestDeviceAuthorization: vi.fn(),
        pollDeviceToken: vi.fn(),
      ...
      const refreshPromise =
        getPrivateProperty<Promise<paramCredentials> | null>(
          tokenManager,
          'refreshPromise',
        );
    ...
    it('should properly clean up timeout when file operation completes before timeout', async () => {
      ...
      ) as (client?: IParamOAuth2Client) => Promise<void>;
