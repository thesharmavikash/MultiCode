...
import { isParamQuotaExceededError } from './quotaErrorDetection.js';
...
export async function retryWithBackoff<T>(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      // Check for param OAuth quota exceeded error - throw immediately without retry
      if (authType === AuthType.PARAM_OAUTH && isParamQuotaExceededError(error)) {
        throw new Error(
          `param OAuth quota exceeded: Your free daily quota has been reached.\n\n` +
            `To continue using param Code without waiting, upgrade to the Alibaba Cloud Coding Plan:\n` +
