...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    ...
    const workspacePath =
      ...
      process.env['param_CODE_IDE_WORKSPACE_PATH'];
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. param Code is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
  ...
  private getPortFromEnv(): string | undefined {
    const port = process.env['param_CODE_IDE_SERVER_PORT'];
    if (!port) {
      return undefined;
    }
    return port;
  ...
  private getStdioConfigFromEnv(): StdioConfig | undefined {
    const command = process.env['param_CODE_IDE_SERVER_STDIO_COMMAND'];
    if (!command) {
      return undefined;
    }

    const argsStr = process.env['param_CODE_IDE_SERVER_STDIO_ARGS'];
    let args: string[] = [];
    if (argsStr) {
      try {
        ...
        if (Array.isArray(parsedArgs)) {
          ...
          debugLogger.error(
            'param_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
          );
        }
      } catch (e) {
        debugLogger.error(
          'Failed to parse param_CODE_IDE_SERVER_STDIO_ARGS:',
          e,
        );
      }
    }
  ...
  private async getLegacyConnectionConfig(
    ...
    if (this.ideProcessInfo) {
      try {
        const portFile = path.join(
          os.tmpdir(),
          `param-code-ide-server-${this.ideProcessInfo.pid}.json`,
        );
        const portFileContents = await fs.promises.readFile(portFile, 'utf8');
        return JSON.parse(portFileContents);
      } catch (_) {
        // For older/newer extension versions, the file name matches the pattern
        // /^param-code-ide-server-${pid}-\d+\.json$/. If multiple IDE
    ...
    if (portFromEnv) {
      try {
        const portFile = path.join(
          os.tmpdir(),
          `param-code-ide-server-${portFromEnv}.json`,
        );
        const portFileContents = await fs.promises.readFile(portFile, 'utf8');
        return JSON.parse(portFileContents);
      } catch (_) {
