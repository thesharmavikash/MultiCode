...
import { PARAM_DIR } from '../config/storage.js';
import {
  ExtensionManager,
  SettingScope,
...
const EXTENSIONS_DIRECTORY_NAME = path.join(PARAM_DIR, 'extensions');
...
function createExtension({
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'PARAM.md'), 'context');
  }
...
describe('extension tests', () => {
  ...
  describe('loadExtension', () => {
    ...
    it('should load context file path when PARAM.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'PARAM.md'),
      ]);
    ...
    it('should use default PARAM.md when contextFileName is empty array', async () => {
      ...
      fs.writeFileSync(path.join(extDir, 'PARAM.md'), 'context content');
      ...
      expect(ext?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext-empty-context', 'PARAM.md'),
      ]);
    });
  ...
  describe('hooks loading and processing', () => {
    it('should load hooks from param-extension.json', async () => {
      const extensionDir = path.join(userExtensionsDir, 'hooks-extension');
      fs.mkdirSync(extensionDir, { recursive: true });

      // Create param-extension.json with hooks
    ...
    it('should load hooks from hooks/hooks.json when not in main config', async () => {
      ...
      // Create param-extension.json without hooks
    ...
    it('should substitute ${CLAUDE_PLUGIN_ROOT} variable in hooks', async () => {
      ...
      // Create param-extension.json with hooks using ${CLAUDE_PLUGIN_ROOT}
      const configWithHooks = {
    ...
    it('should load hooks from config.hooks string path', async () => {
      ...
      // Create param-extension.json with hooks as string path
