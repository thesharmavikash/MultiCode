...
export const EXTENSIONS_DIRECTORY_NAME = path.join(PARAM_DIR, 'extensions');
export const EXTENSIONS_CONFIG_FILENAME = 'param-extension.json';
export const INSTALL_METADATA_FILENAME = '.param-extension-install.json';
  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".param" in markdown files
  ...
  try {
    ...
    for (const file of scriptFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".param" in shell scripts
        // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
        // Avoid matching URLs, comments, or string literals containing .claude
        ...
        if (finalScriptContent !== content) {
          ...
          debugLogger.debug(
            `Updated transcript format and replaced .claude with .param in shell script: ${filePath}`,
          );
        }
      } catch (error) {
        debugLogger.warn(
