...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .paramignore patterns even when not in a git repo', async () => {
      await createTestFile('.paramignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldParamIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldParamIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.paramignore', 'logs/');
    });
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectParamIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectParamIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldParamIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.paramignore', '*.log');
    });
    ...
    it('should return true for qwen-ignored files', () => {
      ...
      expect(
        service.shouldParamIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });
    ...
    it('should return false for non-qwen-ignored files', () => {
      ...
      expect(
        service.shouldParamIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
