...
const EditToolCallContainer: React.FC<ToolCallContainerProps> = ({
  ...
  <div
    className={`param-message message-item ${_className || ''} relative pl-[30px] py-2 select-text toolcall-container toolcall-status-${status}`}
    data-first={isFirst}
    data-last={isLast}
  >
...
export const EditToolCall: React.FC<BaseToolCallProps> = ({
  ...
  if (toolCall.status === 'failed') {
    ...
    return (
      <div
        className={`param-message message-item relative py-2 select-text toolcall-container toolcall-status-${containerStatus}`}
        data-first={isFirst}
        data-last={isLast}
      >
