...
const renderComponent = (
  ...
  const mockConfig = {
    ...
    getModel: vi.fn(() => DEFAULT_PARAM_MODEL),
...
describe('<ModelDialog />', () => {
  ...
  it('initializes with the model from ConfigContext', () => {
    const mockGetModel = vi.fn(() => DEFAULT_PARAM_MODEL);
    renderComponent(
      {},
      {
  ...
  it('initializes with default coder model if getModel returns undefined', () => {
    ...
    // When getModel returns undefined, preferredModel falls back to DEFAULT_PARAM_MODEL
  ...
  it('calls config.switchModel and onClose when DescriptiveRadioButtonSelect.onSelect is triggered', async () => {
    ...
    await childOnSelect(`${AuthType.PARAM_OAUTH}::${DEFAULT_PARAM_MODEL}`);
  ...
  it('updates initialIndex when config context changes', () => {
    const mockGetModel = vi.fn(() => DEFAULT_PARAM_MODEL);
    const mockGetAuthType = vi.fn(() => 'param-oauth');
    const mockSettings = {
      isTrusted: true,
      user: { settings: {} },
      workspace: { settings: {} },
    ...
    // DEFAULT_PARAM_MODEL (coder-model) is at index 0
    expect(mockedSelect.mock.calls[0][0].initialIndex).toBe(0);

    mockGetModel.mockReturnValue(DEFAULT_PARAM_MODEL);
    const newMockConfig = {
      getModel: mockGetModel,
      getAuthType: mockGetAuthType,
