...
import { paramLight } from './param-light.js';
import { paramDark } from './param-dark.js';
...
export const DEFAULT_THEME: Theme = paramDark;
...
class ThemeManager {
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    // Separate param themes
    const paramThemes = builtInThemes.filter(
      (theme) => theme.name === paramLight.name || theme.name === paramDark.name,
    );
    ...
    // Combine param themes first, then sorted others
    return [...paramThemes, ...sortedOtherThemes];
  }
