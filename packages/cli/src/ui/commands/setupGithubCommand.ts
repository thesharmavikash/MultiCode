...
export const GITHUB_WORKFLOW_PATHS = [
  'param-dispatch/param-dispatch.yml',
  'param-assistant/param-invoke.yml',
  'issue-triage/param-triage.yml',
  'issue-triage/param-scheduled-triage.yml',
  'pr-review/param-review.yml',
];
...
// Add param Code specific entries to .gitignore file
export async function updateGitignore(gitRepoRoot: string): Promise<void> {
  const gitignoreEntries = ['.param/', 'gha-creds-*.json'];
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/paramLM/param-code-action/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/paramLM/param-code-action/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
    ...
    return {
      ...
      toolArgs: {
        description:
          'Setting up GitHub Actions to triage issues and review PRs with param.',
        command,
        is_background: false,
      },
    };
