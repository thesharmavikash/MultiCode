...
import { getProjectSummaryPrompt } from '@agent-param/param-core';
...
export const summaryCommand: SlashCommand = {
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    const saveSummaryToDisk = async (
      ...
      // Ensure .param directory exists
      const projectRoot = config.getProjectRoot();
      const paramDir = path.join(projectRoot, '.param');
      try {
        await fsPromises.mkdir(paramDir, { recursive: true });
      } catch (_err) {
        // Directory might already exist, ignore error
      }

      // Save the summary to PROJECT_SUMMARY.md
      const summaryPath = path.join(paramDir, 'PROJECT_SUMMARY.md');
...
`;
      ...
      return {
        filePathForDisplay: '.param/PROJECT_SUMMARY.md',
