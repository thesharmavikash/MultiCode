...
import { IdeClient, AuthType } from '@agent-param/param-core';
 ...
 * For bug reports, removes 'param-' or 'param-code-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'param-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false): string {
  ...
  // For bug reports, remove param- prefix
  if (stripPrefix) {
    return sandbox.replace(/^param-(?:code-)?/, '');
  }
