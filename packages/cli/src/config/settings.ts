...
export const SETTINGS_DIRECTORY_NAME = '.param';
...
export function getSystemSettingsPath(): string {
  if (process.env['param_CODE_SYSTEM_SETTINGS_PATH']) {
    return process.env['param_CODE_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/paramCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\param-code\\settings.json';
  } else {
    return '/etc/param-code/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['param_CODE_SYSTEM_DEFAULTS_PATH']) {
    return process.env['param_CODE_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(settings: Settings, startDir: string): string | null {
  ...
  const userLevelPaths = new Set([
    ...
    path.normalize(path.join(homeDir, PARAM_DIR, '.env')),
  ]);
  ...
  while (true) {
    // Prefer gemini-specific .env under PARAM_DIR
    const geminiEnvPath = path.join(currentDir, PARAM_DIR, '.env');
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeGeminiEnvPath = path.join(homeDir, PARAM_DIR, '.env');
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(PARAM_DIR);
