...
export async function parseArguments(): Promise<CliArgs> {
  ...
  if (
    rawArgv.length > 0 &&
    (rawArgv[0].endsWith('/dist/param-cli/cli.js') ||
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('param')
    .usage(
      'Usage: param [options] [command]\n\nparam Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description: 'Proxy for param Code, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command('$0 [query..]', 'Launch param Code CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
...
export async function loadHierarchicalParamMemory(
  currentWorkingDirectory: string,
  includeDirectoriesToReadParam: readonly string[] = [],
...
export async function loadCliConfig(
  ...
  // Set runtime output directory from settings (env var PARAM_RUNTIME_DIR
  // is auto-detected inside getRuntimeBaseDir() at each call site).
  // Pass cwd so that relative paths like ".param" resolve per-project.
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`param --resume\` without an ID to choose from existing sessions.`;
  ...
  const config = new Config({
    sessionId,
    sessionData,
    embeddingModel: DEFAULT_PARAM_EMBEDDING_MODEL,
