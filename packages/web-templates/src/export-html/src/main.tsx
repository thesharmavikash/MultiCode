...
const logoSvgWithGradient = (() => {
  ...
  const gradientDef =
    '<defs><linearGradient id="param-logo-gradient" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#60a5fa" /><stop offset="100%" stop-color="#a855f7" /></linearGradient></defs>';

  const withDefs = logoSvg.replace(/<svg([^>]*)>/, `<svg$1>${gradientDef}`);

  return withDefs.replace(/fill="[^"]*"/, 'fill="url(#param-logo-gradient)"');
})();
    ...
    <div className="page-wrapper">
      <header className="header">
        <div className="header-left">
          ...
          />
          <div className="logo">
            <div className="logo-text" data-text="param">
              <span className="logo-text-inner">param</span>
            </div>
          </div>
        </div>
      </header>
